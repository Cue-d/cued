...
describe("agent replica worker", () => {
    ...
    function createLegacyReplicaDb() {
    ...
    `);
        db.prepare(`INSERT INTO contacts (id, display_name, company, status, handles_json, updated_at)
       VALUES (?, ?, ?, 'active', ?, ?)`).run("contact-1", "Ava Chen", "Cued", JSON.stringify([
       ...
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run("conv-1", "linkedin", "thread-1", "dm", "Ava Chen", JSON.stringify(["contact-1"]), JSON.stringify(["Ava Chen"]), "Founder update tomorrow?", 1_710_000_000_000);
       ...
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run("msg-1", "conv-1", "Ava Chen", "linkedin", "Founder update tomorrow?", 1_710_000_000_000, "contact-1", "Ava Chen", 0, "delivered", JSON.stringify([{ emoji: "👍", isFromMe: false, timestamp: 1_710_000_000_500 }]));
        db.close();
        return dbPath;
    }
    it("builds a sync bundle from the legacy replica schema", () => {
        ...
        expect(bundle.sourceAccounts).toEqual(expect.arrayContaining([
...
//# sourceMappingURL=agent-replica-worker-lib.test.js.map