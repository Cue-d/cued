...
export function buildFixtureSyncBundle() {
    ...
    const rawEvents = events.map((event) => ({
        ...
        dedupeKey: eventId(`${event.platform}:${event.accountKey}:${event.entityKind}:${event.eventKind}:${event.externalEntityId ?? ""}:${event.conversationExternalId ?? ""}:${event.observedAt}`),
...
//# sourceMappingURL=fixture-data.js.map