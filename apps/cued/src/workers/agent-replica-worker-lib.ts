...
type LegacyConversationRow = {
  ...
  participant_contact_ids_json: string;
...
function isDeterministicHandle(platform: string, type: string, value: string): boolean {
  if (!value.trim()) {
    return false;
  }
...
function normalizeMessageStatus(message: LegacyMessageRow): string {
  if (message.status) {
...
function sourceAccountKeyForPlatform(platform: string, workspaceId: string | null): string {
  return workspaceId ? `${platform}:${workspaceId}` : platform;
}

function resolveLegacyPlatform(platform: string): Platform | null {
  return parsePlatform(platform.trim().toLowerCase());
}

function parseUsageKey(usageKey: string): { platform: Platform | null; accountKey: string } {
...
export function buildAgentReplicaSyncBundle(options?: {
  ...
  try {
    const messages = db
      ...
      .all(messageLimit) as LegacyMessageRow[];
    ...
    const conversations = conversationIds.length
      ? (db
          ...
          .all(JSON.stringify(conversationIds)) as LegacyConversationRow[])
      : [];
    ...
    for (const conversation of conversations) {
      for (const contactId of parseJsonArray<string>(conversation.participant_contact_ids_json)) {
    ...
    const contacts = referencedContactIds.size > 0
      ? (db
          ...
          .all(JSON.stringify([...referencedContactIds])) as LegacyContactRow[])
      : [];

    const conversationById = new Map(conversations.map((conversation) => [conversation.id, conversation]));
    const contactUsageScopes = new Map<string, Set<string>>();
    const contactNameHints = new Map<string, string>();
    const observedBase = Date.now();

    const sourceAccounts = new Map<string, SourceAccountInput>();
    ...
    for (const conversation of conversations) {
      ...
      const participantIds = parseJsonArray<string>(conversation.participant_contact_ids_json);
      const participantNames = parseJsonArray<string>(conversation.participant_names_json);
    ...
    for (const contact of contactsById.values()) {
      const handles = parseJsonArray<LegacyHandle>(contact.handles_json).filter(
        (handle): handle is Required<Pick<LegacyHandle, "platform" | "type" | "value">> =>
          Boolean(handle.platform && handle.type && handle.value),
      );

      const groupedByScope = new Map<string, LegacyHandle[]>();
      ...
      rawEvents.push({
        ...
        dedupeKey: dedupeKey(`agent-replica:conversation:${platform}:${conversation.id}`),
          payload: {
            ...
            participants: parseJsonArray<string>(conversation.participant_contact_ids_json).map((contactId) => ({
              sourceEntityKey: `legacy_contact:${contactId}`,
          })),
        } satisfies ConversationObservationPayload & Record<string, unknown>,
        sourceVersion: "agent-replica-v1",
      });
    ...
    for (const [index, message] of orderedMessages.entries()) {
      ...
      rawEvents.push({
        ...
        } satisfies MessagePayload & Record<string, unknown>,
        sourceVersion: "agent-replica-v1",
      });
      messageObservedAt += 1;

      const reactions = parseJsonArray<LegacyReaction>(message.reactions_json);
