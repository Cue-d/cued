...
function loadContactsFromFixture(path) {
...
function loadContactsFromNativeBinary(path) {
    const stdout = execFileSync(path, ["contacts", "dump"], {
        encoding: "utf8",
    });
    return JSON.parse(stdout);
}
function loadContactsFromMacOS() {
    const script = `
...
export function resolveContactsLoader(env = process.env, repoRoot) {
...
export function buildContactsSyncBundle() {
    const loader = resolveContactsLoader();
    const contacts = loader.kind === "fixture"
    ...
    return {
        ...
        rawEvents: contacts.map((contact, index) => ({
            ...
            dedupeKey: dedupeKey(`contacts:${contact.sourceId}:${contact.displayName}:${(contact.phoneNumbers ?? []).join(",")}:${(contact.emails ?? []).join(",")}`),
            payload: {
...
//# sourceMappingURL=contacts-worker-lib.js.map