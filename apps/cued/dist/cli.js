...
import { buildIntegrationStatus, connectIntegration, disconnectIntegration, getAuthSessionSummary, listIntegrationStates, listRequestableIntegrationPlatforms, completeAuthSession, getIntegrationSummary, markAuthSessionInProgress, refreshManagedIntegrationStates, setIntegrationEnabled, } from "./integrations/service.js";
...
async function handleLocalIntegrationCommand(subcommand, rest) {
    ...
    try {
        switch (subcommand) {
            ...
            default:
                throw new Error(`Usage: cued integrations list | status | refresh | connect <platform> [account] | disconnect <platform> [account] | auth-status <session-id> | auth-cancel <session-id> | enable <platform> [account] | disable <platform> [account] | login <platform> [account]\nRequestable platforms: ${listRequestableIntegrationPlatforms().join(", ")}`);
        }
    }
    finally {
        db.close();
    ...
    if (command === "hooks") {
        switch (subcommand) {
            ...
            case "doctor":
                ...
                return;
            case "test":
                if (!rest[0] || !HOOK_EVENT_NAMES.includes(rest[0])) {
                    throw new Error(`Usage: cued hooks test <event>\nEvents: ${HOOK_EVENT_NAMES.join(", ")}`);
                }
                printJson(await testHookEvent(rest[0]));
                return;
            default:
