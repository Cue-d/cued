...
describe("CuedDatabase", () => {
  ...
  it("treats legacy attachment migration ids as compatible with the current numbering", () => {
    const dir = mkdtempSync(join(tmpdir(), "cued-v2-db-legacy-attachment-migration-"));
    tempDirs.push(dir);
    const dbPath = join(dir, "local.db");
    const sqlite = new Database(dbPath);
    ...
    for (const migration of MIGRATIONS) {
      if (
        migration.id === "0011_sync_run_timing_v2" ||
        migration.id === "0012_attachment_access_and_content"
      ) {
        continue;
      }
      sqlite
    ...
    sqlite
      .prepare("INSERT INTO schema_migrations (id, applied_at) VALUES (?, ?)")
      .run("0011_attachment_access_and_content", appliedAt);
    ...
    sqlite
      ...
      .run("completed-run", "whatsapp", "default", "sync", "completed", "manual", 200, 250, "{}");
    sqlite.close();
    ...
    const migrationIds = migratedSqlite
      .prepare("SELECT id FROM schema_migrations WHERE id LIKE '001%' ORDER BY id ASC")
      .all()
      .map((row) => row.id);
    const migratedRun = migratedSqlite
      .prepare("SELECT id, queued_at, started_at FROM sync_runs WHERE id = ?")
      .get("completed-run") as { id: string; queued_at: number; started_at: number | null };
    const attachmentColumns = migratedSqlite
      .prepare("PRAGMA table_info(message_attachments)")
      .all()
      .map((row) => row.name);

    expect(migrationIds).toContain("0011_attachment_access_and_content");
    expect(migrationIds).toContain("0011_sync_run_timing_v2");
    expect(migrationIds).toContain("0012_attachment_access_and_content");
    expect(migratedRun).toEqual({
      id: "completed-run",
      queued_at: 200,
      started_at: 200,
    });
    expect(attachmentColumns.filter((name) => name === "access_kind")).toHaveLength(1);
