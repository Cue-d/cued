...
function parsePicture(profilePicture) {
    const vectorImage = profilePicture?.displayImageReference?.vectorImage;
    if (!vectorImage?.rootUrl || !vectorImage.artifacts?.length) {
        return undefined;
    }
    const largest = vectorImage.artifacts.reduce((left, right) => right.width > left.width ? right : left);
    return {
...
function parseConnectionsResponse(response) {
    ...
    for (const element of response.included) {
        if (element.$type === "com.linkedin.voyager.dash.identity.profile.Profile" && element.entityUrn) {
            profileMap.set(element.entityUrn, element);
        }
    }
    const connections = [];
    for (const element of response.included) {
        ...
        const memberRef = element.connectedMemberResolutionResult
...
export async function getConnections(client, cursor) {
