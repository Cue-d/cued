...
import { contactFieldValues, contactHandles, contactObservations, contactSources, contacts, conversationObservations, conversationParticipants, conversations, messageEvents, messageReactions, messages, } from "../db/schema.js";
import { isContactFieldName, } from "../types/provider.js";
function hashId(prefix, value) {
    return `${prefix}_${createHash("sha256").update(value).digest("hex").slice(0, 24)}`;
}
function normalizeHandle(type, value) {
...
function fieldPriority(platform, fieldName) {
...
function currentBestPriority(conn, contactId, fieldName) {
    const row = conn
        .select({
        ...
        .where(and(eq(contactFieldValues.contactId, contactId), eq(contactFieldValues.fieldName, fieldName), eq(contactFieldValues.isCurrentBest, 1)))
        .limit(1)
        .get();
    return row ?? null;
}
export function rebuildProjectedState(db) {
    const rawEvents = db.listRawEvents();
    db.clearProjectedState();
    const sourceContactMap = new Map();
    const deterministicHandleMap = new Map();
...
function projectContactObservation(conn, sourceContactMap, deterministicHandleMap, event) {
    ...
    const existingContactId = deterministicHandles
        ...
        .find((contactId) => Boolean(contactId));
...
function projectConversationObservation(conn, sourceContactMap, conversationMap, event) {
    const payload = JSON.parse(event.payload_json);
...
function projectMessageEvent(conn, sourceContactMap, conversationMap, event) {
    const payload = JSON.parse(event.payload_json);
...
function projectReactionEvent(conn, sourceContactMap, conversationMap, event) {
    const payload = JSON.parse(event.payload_json);
    const messageId = hashId("message", `${event.platform}:${event.account_key}:${payload.sourceMessageKey}`);
    const conversationId = conversationMap.get(`${event.platform}:${event.account_key}:${payload.sourceConversationKey}`)
        ?? hashId("conversation", `${event.platform}:${event.account_key}:${payload.sourceConversationKey}`);
    ...
    const row = conn
        ...
        .where(and(eq(messageReactions.messageId, messageId), eq(messageReactions.isActive, 1)))
        .get();
...
function reindexMessagesFts(conn) {
    conn.run(sql.raw("DELETE FROM messages_fts"));
    const indexedMessages = conn.all(sql `
    SELECT
      m.id,
      m.platform,
      m.content_current,
...
function refreshConversationDisplayNames(conn) {
    conn.run(sql `
